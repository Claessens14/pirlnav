...
config="configs/experiments/il_objectnav-eval-powerPC.yaml"
...
EVAL_CKPT_PATH_DIR="data/checkpoints/pretrained/objectnav_il_hd"
...
python -u -m run --exp-config $config --run-type eval TENSORBOARD_DIR $TENSORBOARD_DIR EVAL_CKPT_PATH_DIR $EVAL_CKPT_PATH_DIR NUM_ENVIRONMENTS 1 RL.DDPPO.force_distributed True TASK_CONFIG.DATASET.SPLIT "val"  TASK_CONFIG.DATASET.DATA_PATH "$DATA_PATH/{split}/{split}.json.gz" 
